...
import { connectUserPhoneNumber } from 'graphql/client/mutations/auth'
...
export default function ConnectUserPhoneNumber ({ phoneNumber, message, onClose }) {
  ...
  const { mutate, data: successData, isLoading, error } = useMutation(connectUserPhoneNumber)

  const handleCreateUserClose = async (userID) => {
    setIsAddNewOpen('')
    if (userID) {
      await handleConnectUserPhoneNumber(userID)
    }
  }

  const handleConnectUserPhoneNumber = async (userID) => {
    const token = await firebaseAuth.currentUser.getIdToken()
    mutate({ userID, phoneNumber, token })
  }
  ...
  const handleOnChange = async userRelation => {
    if (userRelation.__isNew__) {
      ...
      handleConnectUserPhoneNumber(userID)
    }
  }
            ...
            {!successData && (
              <Stack minH='500'>
                ...
                <Text
                  ...
                  borderRadius='10px'
                >
                  Please select an existing user to connect with this phone number
                </Text>
                <UserSelection
